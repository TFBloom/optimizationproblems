...
We define $C_{24}$ to be the smallest integer $n\equiv 0 \pmod{4}$ such that there is **no** Hadamard matrix of order $n$.
If no such $n$ exists, we set $C_{24}=\infty$.

The **Hadamard conjecture** asserts that $C_{24}=\infty$, i.e. that Hadamard matrices exist for every order $n\equiv 0 \pmod{4}$.
...
| Bound | Reference | Comments |
| ----- | --------- | -------- |
| $4$ | Trivial | By definition $C_{24}$ (if finite) is a multiple of $4$. |
